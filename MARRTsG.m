classdef MARRTsG
    ...
    methods
        function rrt = MARRTsG(map,position,goal,Agent_count)                           % 初始化RRT算法
            %UNTITLED2 构造此类的实例
            %   此处显示详细说明
        ...
        function rrt = runMARRTsG(rrt,single_solution)                                % 运行RRT算法
...
%                     rrt.numnodes = num_nodes;
                    rrt.runtime = run_time;
                    rrt.bestsol = best_solution;
                    rrt.firstsol = first_solution;
                 	return;
                end
                ...
                if isequal(goal_index,0)                                        % 找到初始的路径，以后每次迭代不重新计算路径
                    ...
                    for i = 1:size(rrt.nodes,1)                                             % 对树中的每一个节点
                        if isequal(rrt.nodes(i,:),rrt.goal)                                 % 如果该节点和目标节点相等
                            ...
                            rrt.path = Path;                                                % 最终得到的路径存储在rrt的path中
                            first_solution = COST(rrt,rrt.parent,rrt.cost,goal_index);
                            fprintf('number of iterations = %d, runtime = %d, cost of path = %d\n',iterations,run_time,first_solution);
                            flag = 1;
                            break;
                        end
                    end
...
%                     child_nodes = children(rrt,A(j).map,x_j(1),x_j(2));             % 找到其在网格图中的孩子（相邻）节点并入相邻节点表
                    ...
                    if ~isempty(child_nodes)                                        % 如果孩子节点不为空则将最小H值孩子添加为路径点
                        ...
                        for k = 1:size(child_nodes,1)                                              % 对每个孩子节点
                            ...
                            H_value = sum(distance) + Map(child_nodes(k,1),child_nodes(k,2),j) + rand; % 计算标量总距离加上小于1的偏移预防H值相等
...
%                         x_min = minHvalueNode(rrt,child_nodes,dest_j);              % 找到孩子节点集表中启发函数值最小的节点 
                        ...
                        Map(x_min(1),x_min(2),j) = Map(x_min(1),x_min(2),j) + 1;    % 该路径点对应的地图值+1
                        c = c + 1;                                                  % 代价加1
